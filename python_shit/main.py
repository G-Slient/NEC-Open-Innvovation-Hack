...
app = Flask(__name__)
client = pymongo.MongoClient("mongodb://admin:admin@localhost:27017/item_catalog")
...
def inventory_sub(store_id, item_ids, quantity):
    db = client['item_catalog']
    col = db[store_id]
    for ind, item_id in enumerate(item_ids):
        existing = col.find_one({"item_id": item_id}, {"quantity": 1})['quantity']
        current_neg = quantity[ind]
        if store_id == "store_1":
            db.store_1.update_one({"item_id": item_id}, {"$set": {"quantity": existing - current_neg}}, upsert=False)
        elif store_id == "store_2":
            db.store_2.update_one({"item_id": item_id}, {"$set": {"quantity": existing - current_neg}}, upsert=False)
        elif store_id == "store_3":
            db.store_3.update_one({"item_id": item_id}, {"$set": {"quantity": existing - current_neg}}, upsert=False)
        elif store_id == "store_4":
            db.store_4.update_one({"item_id": item_id}, {"$set": {"quantity": existing - current_neg}}, upsert=False)
        elif store_id == "store_5":
            db.store_5.update_one({"item_id": item_id}, {"$set": {"quantity": existing - current_neg}}, upsert=False)
...
def transaction_complete():
    ...
    if request.method == 'GET':
        ...
        # inventory_sub(store_id, item_list, quantity)
